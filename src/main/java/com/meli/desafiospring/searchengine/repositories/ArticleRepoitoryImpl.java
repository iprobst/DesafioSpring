...
public class ArticleRepoitoryImpl implements ArticleRepository {
    ...
    public void updateDataBase(ArticleDTO updateArticle){
    ...
    private List<ArticleDTO> loadDataBase() {
        ...
        try {
            file = ResourceUtils.getFile("classpath:articless.json");
...
//        objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
    ...
    private void saveDataBase() {
        ...
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            objectMapper.writeValue(new File("src/main/resources/articless.json"), this.database);
//            objectMapper.writeValue(new File("target/classes/com/articless.json"), this.database);
        } catch (IOException e) {
            e.printStackTrace();
        }
